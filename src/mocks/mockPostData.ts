...
export const generateMockPosts = (count: number) => {
  return Array.from({ length: count }, (_, index) => ({
    ...
    SeasonTag: SEASON_TAGS.map((tag) => tag.id)[Math.floor(Math.random() * 4)],
    WeatherTags: WEATHER_TAGS.map((tag) => tag.id).slice(0, Math.floor(Math.random() * 3) + 1),
    TemperatureTags: TEMPERATURE_TAGS.map((tag) => tag.id).slice(0, Math.floor(Math.random() * 3) + 1),
