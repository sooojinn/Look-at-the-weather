...
interface LocationTermsCheckBoxProps {
  ...
  setIsTermsChecked: (value: React.SetStateAction<boolean>) => void;
}

export default function LocationTermsCheckBox({
  isTermsChecked: isChecked,
  setIsTermsChecked: setIsChecked,
}: LocationTermsCheckBoxProps) {
    ...
    <div>
      <div className="h-12 flex justify-between items-center cursor-pointer">
        <label htmlFor="terms" className="cursor-pointer flex items-center">
          <input type="checkbox" id="terms" className="hidden" checked={isChecked} onClick={handleCheckboxChange} />
